...
class RPEIndexFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, index):
        ...
        Parameters
        ----------
        input: torch.Tensor, np.float32
            The shape is (B, H, L_query, num_buckets)
        index: torch.Tensor, np.int32
    ...
    def backward(ctx, grad_output):
        '''
          - Inputs
              grad_output: np.float32 (B, H, L_query, L_key)
          - Outputs
              grad_input: np.float32 (B, H, L_query, num_buckets)
        '''
